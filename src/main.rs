...
fn main() {
    ...
    let gtfs = if opt.input.starts_with("http") {
        gtfs_structures::Gtfs::from_url(&opt.input)
    } else if opt.input.to_lowercase().ends_with(".zip") {
        gtfs_structures::Gtfs::from_zip(&opt.input)
    } else {
        gtfs_structures::Gtfs::new(&opt.input)
    };

    let validation = gtfs.map(|gtfs| validators::validate(&gtfs))
