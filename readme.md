...
| NegativeStopDuration             | Error       | The `departure_time` at a stop is earlier than its `arrival_time`, or the `arrival_time` at a stop is earlier than the `departure_time` of the previous stop in the same trip.           |
| MissingArrivalOrDepartureTime    | Warning     | A stop time has only one of `arrival_time` or `departure_time` defined (both or neither are expected).                                                                                   |
